//! Simple, robust progressive streaming implementation.
//!
//! This module implements a radically simplified approach to progressive streaming
//! that prioritizes correctness, testability, and debuggability over complex async
//! coordination.
...
impl StreamPump {
    ...
    pub fn pump_to<W: Write>(&self, mut writer: W) -> Result<u64, SimpleStreamError> {
        let mut offset = 0u64;
        ...
        while offset < MIN_HEADER_SIZE.min(self.file_size) {
            if wait_start.elapsed() > Duration::from_secs(60) {
                return Err(SimpleStreamError::InitialDataTimeout);
            }
            ...
            match self.read_chunk(offset, (offset + CHUNK_SIZE).min(self.file_size)) {
                Ok(data) if !data.is_empty() => {
                    writer
                        .write_all(&data)
                        .map_err(SimpleStreamError::WriteFailed)?;
...
impl FfmpegRunner {
    ...
    pub fn run_progressive(&self) -> Result<FfmpegHandle, SimpleStreamError> {
        ...
        let mut child = cmd.spawn().map_err(SimpleStreamError::FfmpegStart)?;
        ...
        let stdin = child.stdin.take().ok_or(SimpleStreamError::NoStdin)?;
...
/// Simple progressive streaming coordinator.
pub struct SimpleProgressiveStreamer {
    pump: StreamPump,
    runner: FfmpegRunner,
}

impl SimpleProgressiveStreamer {
    ...
    pub fn start(self) -> JoinHandle<Result<(), SimpleStreamError>> {
        tokio::task::spawn_blocking(move || {
            ...
            match pump_handle.join() {
                ...
                Err(_) => {
                    ...
                    return Err(SimpleStreamError::WriteFailed(io::Error::other(
            ...
            let status = waiter.wait().map_err(SimpleStreamError::FfmpegStart)?;

            if !status.success() {
                return Err(SimpleStreamError::FfmpegFailed(status));
            }
