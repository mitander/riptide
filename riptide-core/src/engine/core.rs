...
use crate::torrent::tracker::{AnnounceEvent, AnnounceRequest};
use crate::torrent::{
    AdaptivePiecePicker, AdaptiveStreamingPiecePicker, BencodeTorrentParser, BufferStatus,
    InfoHash, PeerId, PeerManager, PiecePicker, TorrentError, TorrentParser, TrackerManager,
};
...
struct DownloadContext<T: TrackerManager, P: PeerManager> {
    info_hash: InfoHash,
    metadata: TorrentMetadata,
...
impl<P: PeerManager + 'static, T: TrackerManager + 'static> TorrentEngine<P, T> {
    ...
    pub async fn start_download(&mut self, info_hash: InfoHash) -> Result<(), TorrentError> {
        ...
        self.configure_upload_for_streaming(info_hash, metadata.piece_length as u64)
            .await;
    ...
    async fn discover_peers(
        ...
        let mut manager = tracker.write().await;
    ...
    async fn download_pieces(
        ...
        tracing::info!(
            ...
            params.piece_count,
            params.peer_addresses.len()
        );

        let peers_for_stats = Arc::clone(&params.peers);
        ...
        piece_downloader
            .update_peers(params.peer_addresses.clone())
            .await;
        tracing::debug!(
            "Updated piece downloader with {} peers",
            params.peer_addresses.len()
        );
        tracing::trace!("Peer addresses: {:?}", params.peer_addresses);
            ...
            match piece_downloader.download_piece(piece_idx).await {
                Ok(()) => {
                    ...
                    if Self::should_update_stats(
                        ...
                        Self::calculate_and_notify_stats(
                            params.info_hash,
                            &peers_for_stats,
                            start_time,
                            bytes_downloaded,
    ...
    fn should_update_stats(
        now: Instant,
        last_speed_update: Instant,
        pieces_downloaded: u32,
        piece_count: u32,
    ) -> bool {
        now.duration_since(last_speed_update) >= Duration::from_secs(2)
            || pieces_downloaded == piece_count
    }

    async fn calculate_and_notify_stats(
        info_hash: InfoHash,
        peers_for_stats: &Arc<RwLock<P>>,
        start_time: Instant,
        ...
        let (bytes_uploaded, upload_speed_bps) = {
            let peers = peers_for_stats.read().await;
            peers.upload_stats().await
        };
    ...
    pub async fn download_statistics(&self) -> EngineStats {
        ...
        let total_peers = {
            ...
            // Calculate total peers across all torrents
            peers.connected_peers().await.len()
        };
        ...
        let total_uploaded = {
            let peers = self.peers.read().await;
            let (bytes_uploaded, _speed) = peers.upload_stats().await;
            bytes_uploaded
        };
    ...
    ///
    /// Uses the Peers trait interface to configure upload throttling
    /// without requiring downcasting to specific implementation types.
    pub async fn configure_upload_for_streaming(&self, info_hash: InfoHash, piece_size: u64) {
        ...
        if let Err(e) = peers
            .configure_upload(info_hash, piece_size, total_bandwidth)
            .await
        {
            tracing::warn!(
