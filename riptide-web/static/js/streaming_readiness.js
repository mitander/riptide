...
class EnhancedVideoPlayer {
  ...
  setupVideoEventHandlers() {
    // Handle video errors - log but don't interfere with recovery
    this.video.addEventListener("error", (e) => {
      console.warn("Video error event:", e);
      // Don't show persistent error overlay - let it recover naturally
    });

    // Clear status when video can play
    this.video.addEventListener("canplay", () => {
      console.log("Video can play - hiding status");
      this.hideStatus();
      this.isRecovering = false;
      this.clearErrorOverlays();
    });
    ...
    // Handle seeking without showing errors
    ...
    // Handle stalled/waiting events gracefully
    ...
    // Clear any error overlays when video starts playing
    this.video.addEventListener("playing", () => {
      console.log("Video playing - ensuring clean state");
      this.hideStatus();
      this.clearErrorOverlays();
    });
  }
  ...
  /**
   * Gentle error recovery without reloading
   */
  gentleErrorRecovery() {
    console.log("Attempting gentle error recovery");

    // Only clear visual error overlays, don't touch the video element
    this.clearErrorOverlays();
  ...
  /**
   * Clear browser's native error overlay UI elements
   */
  clearErrorOverlays() {
    // Only remove error text overlays that browsers might add
    const videoContainer = this.video.parentElement;
    if (videoContainer) {
      const errorElements = videoContainer.querySelectorAll(
        '[class*="error"], [class*="Error"], [class*="message"]',
      );
      errorElements.forEach((el) => {
        if (
          el !== this.statusContainer &&
          el.textContent.includes("network error")
        ) {
          console.log("Hiding browser error overlay");
  ...
  async loadVideo() {
    ...
    this.readinessChecker = new StreamingReadinessChecker(this.infoHash, {
      ...
      onProgress: (readiness, retry, maxRetries) =>
        this.onStreamProgress(readiness, retry, maxRetries),
  ...
  onStreamReady(readiness) {
    ...
    // Clear any existing error UI overlays
    this.clearErrorOverlays();
    ...
    // Status will be hidden by canplay event handler
  }
...
function initEnhancedVideoPlayer() {
  ...
  const infoHash =
    pathParts[pathParts.length - 1] || videoElement.dataset.infoHash;
