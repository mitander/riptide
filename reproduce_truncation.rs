...
async fn monitor_progressive_growth(info_hash: &str, duration_minutes: u64) -> Result<Vec<Measurement>, Box<dyn std::error::Error>> {
    ...
    println!("Monitoring progressive streaming for {} minutes...", duration_minutes);
...
fn analyze_results(measurements: &[Measurement], original_file_size: u64) {
    if measurements.is_empty() {
        println!("ERROR: No measurements to analyze");
        return;
    }
    ...
    println!("\nANALYSIS RESULTS:");
    println!("================================================================");
    println!("Original file size: {} bytes ({:.1} MB)", 
        format_number(original_file_size), original_file_size as f64 / 1024.0 / 1024.0);
    println!("Final stream size:  {} bytes ({:.1} MB)", 
        format_number(final_size), final_size as f64 / 1024.0 / 1024.0);
    println!("Percentage served:  {:.1}%", percentage_served);
    
    if percentage_served < 95.0 {
        println!("TRUNCATION DETECTED: Only {:.1}% of file was served", percentage_served);
        ...
        println!("Estimated served duration: {}:{:02}", served_minutes, served_seconds);
        
        // This is the exact issue described - progressive streaming is stopping early
        println!("\nROOT CAUSE ANALYSIS:");
        println!("- Progressive streaming starts correctly");
        println!("- Stream grows initially but then stops");
        println!("- Only {:.1}% of 16min 20s movie is served", percentage_served);
        println!("- This explains why users see '1min 15sec movie' from full-length film");
    } else {
        println!("SUCCESS: Full file appears to be served");
    }
    
    // Find when growth stopped
    if let Some(last_growth) = measurements.iter().filter(|m| m.growth_bytes > 0).last() {
        println!("Streaming stopped growing after: {} seconds", last_growth.elapsed_seconds);
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = async {
        ...
        // Analyze results
        let original_file_size = 135046574; // From logs: "Final file size determined: 135046574 bytes"
